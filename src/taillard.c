#define ANSI_C 1     /* 0:   K&R function style convention */
...
#if ANSI_C == 1
void write_problem(short p, char * buffer)  /* write out problem */ 
#else
void write_problem(p,buffer)
short p;
char * buffer;
#endif
{
  ...
  char *endung=(char*)calloc(255,sizeof(char));
  
	endung[0]='b';
	endung[1]='\0';
	endung=(char*)strcat(endung,(buffer));
	endung=(char*)strcat(endung,".dat");
  sprintf(name,"ta%03d", p);                 /* file name construction */ 
  name=(char*)strcat(name,endung);	//Dateiname erstellen
  if(!(f = fopen(name,"w"))) {               /* open file for writing  */ 
    fprintf(stderr,"file %s error\n", name);
    return;
  }
  fprintf(f,"%d %d\n",  S[p].num_mach, S[p].num_jobs); /* write header line */ 
  
  if (strcmp(buffer,"n")==0)	{			//Buffer 
				buffergroesse=S[p].num_jobs-1;
			}
			else	{
				buffergroesse=atoi(buffer);
			}
			for(i=0; i<S[p].num_mach-1; i++)	{	//Buffer erzeugen
				fprintf(f,"%3d ",buffergroesse);
			}
  fprintf(f,"\n");
  for(j = 0; j < S[p].num_jobs; ++j) {
    for(i = 0; i < S[p].num_mach; ++i) {
       fprintf(f," %2d ", d[i][j]);   /* write machine and job */ 
    }
    fprintf(f,"\n");                         /* newline == End of job */ 
  }
  fclose(f);                                 /* close file */ 
...
int main(int argc, char ** argv)                                    
{
  ...
  if (argc!=2)	{
  	fprintf(stderr,"usage: %s buffersize\n",argv[0]);
		fprintf(stderr,"Buffersize may be an integer or n\n");
		fprintf(stderr,"If n is typed, then buffersize will be number_of_machines-1\n");
	}
	else	{
	  while(S[i].rand_time) {                    /* for i == 1 up to NULL entry */
	    generate_flow_shop(i);                   /* generate problem i  */ 
	    write_problem(i,argv[1]);                        /* write out problem i */ 
	    ++i;                                     /* increment i */ 
	  }
  }
  return 0;
